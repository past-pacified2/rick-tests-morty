...
import { copyForStatus, type ErrorCopy, NOT_FOUND, REQUEST_FAILED, UNEXPECTED } from './errors';
...
describe('copyForStatus', () => {
  ...
  it('treats every non-404 status below 500 as unexpected', () => {
    fc.assert(
      fc.property(
        fc.integer({ min: 100, max: 499 }).filter((status) => status !== 404),
        (status) => {
...
describe('the copy itself', () => {
  ...
  it('never mentions internals a user cannot act on', () => {
    ...
    for (const copy of [NOT_FOUND, REQUEST_FAILED, UNEXPECTED]) {
